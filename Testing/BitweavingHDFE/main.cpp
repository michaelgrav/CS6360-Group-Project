...
class BitWeavingH {
private:
    std::vector<std::bitset<ENCODED_INT_BITS>> rows;
    size_t numValues;

public:
    BitWeavingH(size_t numValues)
            : numValues(numValues), rows(numValues) {}

    void addValue(uint32_t value, size_t colIndex) {
        for (size_t bitIndex = 0; bitIndex < ENCODED_INT_BITS; ++bitIndex) {
            rows[colIndex][bitIndex] = (value & (1 << bitIndex)) != 0;
        }
    }

    std::vector<size_t> searchLessThan(uint32_t threshold) {
        std::vector<size_t> resultIndices;
        for (size_t colIndex = 0; colIndex < numValues; ++colIndex) {
            uint32_t value = 0;
            for (size_t bitIndex = 0; bitIndex < ENCODED_INT_BITS; ++bitIndex) {
                if (rows[colIndex][bitIndex]) {
                    value |= (1 << bitIndex);
                }
            }
            if (value < threshold) {
                resultIndices.push_back(colIndex);
            }
        }
...
int main() {
    const std::string filename = "/Users/yuliang/CLionProjects/BitweavingHDFE/donations-compressed-dfe";
    ...
    BitWeavingH bitWeaving(numValues);
